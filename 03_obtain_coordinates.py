...
dbd_min = (
    dbd_df[["Protein_ID", "Pfam", "From", "To", "Aas"]]
...
# Verificar las que hicieron match
df_long["Aas"] = df_long["Aas"].astype("string").str.strip()

# Comparación exacta
df_long["DBD_match"] = (
    df_long["DBD_seq"].notna()
    & df_long["Aas"].notna()
    & (df_long["DBD_seq"] == df_long["Aas"])
)

total_con_Aas = df_long["Aas"].notna().sum()
matches = df_long["DBD_match"].sum()
print("Total con Aas:", total_con_Aas)
print("Matches exactos:", matches)
print("Mismatches:", total_con_Aas - matches)
print("Porcentaje de acierto:", matches / total_con_Aas * 100 if total_con_Aas else 0)
