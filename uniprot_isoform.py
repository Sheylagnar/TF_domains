...
def uniprot_base_from_ensp(ensp: str):
    ...
    job = S.post(
        f"{UNI}/idmapping/run",
        data={"from": "Ensembl_Protein", "to": "UniProtKB", "ids": ensp},
        timeout=30,
    )
    ...
    while True:
        ...
        if st.get("jobStatus") in (None, "FINISHED"):
            break
        time.sleep(0.4)
...
def uniprot_isoforms(base_acc: str):
    ...
    for c in entry.get("comments", []):
        if (c.get("commentType") or c.get("type")) in (
...
def fetch_uniprot_fasta(acc_or_iso: str):
    ...
    if r.status_code != 200:
        return None
    return "".join(
        ln.strip() for ln in r.text.splitlines() if ln and not ln.startswith(">")
    )
...
def add_uniprot_isoform_with_progress(
    df: pd.DataFrame,
...
tf_2 = add_uniprot_isoform_with_progress(
    tf_2, col_in="Transcript ID", col_out="Uniprot_isoform", empty_label=""
)
